  ...
  return (
    <aside className="sidebar">
      ...
      <nav className="sidebar-nav">
        ...
        {/* Library Section */}
        <div className="sidebar-section">
          <div className="sidebar-section-title">Deine Bibliothek</div>

          <button
            onClick={onLikedSongs}
            className="sidebar-item w-full"
          >
            <div className="sidebar-item-icon w-6 h-6 rounded-sm bg-gradient-to-br from-[#4000BF] to-[#B5179E] flex items-center justify-center">
            ...
            <div className="sidebar-playlists">
              {playlists.map((playlist) => (
                <button
                  ...
                  className="sidebar-item w-full"
                >
                  <div className="sidebar-item-icon w-6 h-6 rounded-sm bg-[#282828] flex items-center justify-center flex-shrink-0">
      ...
      <div className="sidebar-create-playlist">
        <button onClick={onUpload} className="sidebar-item w-full">
          <Upload className="sidebar-item-icon" />
          <span>Musik hochladen</span>
        </button>
        <button onClick={() => onTabChange('youtube')} className="sidebar-item w-full">
          <Youtube className="sidebar-item-icon" />
          <span>YouTube Download</span>
        </button>
        <button onClick={() => onTabChange('aistudio')} className="sidebar-item w-full">
          <Sparkles className="sidebar-item-icon" />
          <span>KI Studio</span>
        </button>
      </div>
...
export function BottomNav({ active, onChange, likedCount }: {
  active: NavTab;
  ...
  const tabs: { id: NavTab; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'liked', label: likedCount ? `Liked (${likedCount})` : 'Liked', icon: <Heart className="h-6 w-6" /> },
...
export function MiniPlayer({ file, token, isPlaying, progress, onToggle, onOpen }: {
  ...
  return (
    <div className="mini-player" role="button" tabIndex={0} onClick={onOpen} onKeyDown={(e) => e.key === 'Enter' && onOpen()}>
      <img className="mini-player-cover" src={coverUrl(file, { token, artist: file?.artist, album: file?.album })} alt="" />
