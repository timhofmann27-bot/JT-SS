...
export function FullscreenPlayer({
  ...
  const handleDownload = useCallback(() => {
    ...
    const streamUrl = url.replace('/api/cover/', '/api/stream/').split('?')[0] + `?token=${encodeURIComponent(token)}`;
  ...
  return (
    ...
    >
      <div className="spotify-player-bg">
        {file?.hasArtwork && (
          <div
            className="spotify-player-blur"
            style={{ backgroundImage: `url(${coverUrl(file, { token, artist: file?.artist, album: file?.album })})` }}
          />
        )}
        <div style={{
          position: 'absolute',
          inset: 0,
          background: `radial-gradient(circle at 50% 30%, ${hue}40 0%, #121212 70%)`,
        }} />
      </div>
              ...
              <AnimatePresence>
                {showMore && (
                  <>
                    ...
                    >
                      <button
                        onClick={() => { onOpenQueue(); setShowMore(false); }}
                        style={{
                      ...
                      >
                        <ListOrdered style={{ width: 18, height: 18 }} /> Zur Warteschlange
                      </button>
                      <button
                      ...
                      <button
                        ...
                        onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}
                      >
                        <Share2 style={{ width: 18, height: 18 }} /> Teilen
                      </button>
                    </motion.div>
                  </>
                )}
